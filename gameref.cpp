...
//(RANDY)
void ChessGame::check_pawn_conversion(Position end) {
...
int ChessGame::make_move(Position start, Position end) {
  ...
  /*
  if (move_type > 0) {
    cout << "why is this printing" << endl; 
    //check_pawn_conversion(end); 
  }
  */
  return move_type;
}
...
int ChessGame::checkmate() {
  ...
  if (!valid_adjacent_king_move() && !causing_check_piece_capturable() && !blockable_check()) {
    return 1;
  }
  return 0;
}

int ChessGame::blockable_check() {
  ...
  for (unsigned i = 0; i < _pieces.size(); i++) {
    if (_pieces.at(i) != nullptr && _pieces.at(i)->owner() == player_turn()) {
      Position start;
      start.x = i % _width;
      start.y = i / _width; 
      ...
      for (unsigned j = 1; j < trajectory.size() - 1; j++) {
	int val = valid_move_given_board(start, trajectory.at(j), player_turn());
	if (val >= 0 && check_for_check(start, trajectory.at(j), player_turn()) >= 0) {
	  return 1;
	}
      }
    }
...
int ChessGame::causing_check_piece_capturable() {
  for (unsigned i = 0; i < _pieces.size(); i++) {
    if (_pieces.at(i) != nullptr && _pieces.at(i)->owner() == player_turn()) {
      Position start;
      start.x = i % _width;
      start.y = i / _width; 
      int val = valid_move_given_board(start, causing_check, player_turn());
      if (val >= 0 && check_for_check(start, causing_check, player_turn()) >= 0) {
	return 1;
      }
    }
  }
...
int ChessGame::valid_adjacent_king_move() {
  Position adjacent;
  int val;
  if (player_turn() == WHITE) {
...
void ChessGame::print_move_prompt(int return_message) {
  ...
  case 4:
    ...
    Prompts::win(not_player_turn(), _turn);
    Prompts::game_over();
    break;
  case 5:
    Prompts::stalemate();
  ...
  case 6:
    ...
    break; 
  case 7:
    Prompts::win(not_player_turn(), turn()); break;
  case 8:
...
//#####################NOTE#############################
//remember to delete the piece that is going to be captured.
///######/######/######/######/######/######/############
int ChessGame::check() {
  for (unsigned i = 0; i < _pieces.size(); i++) {
    if (_pieces.at(i) != nullptr && _pieces.at(i)->owner() == not_player_turn()) {
      vector<Position> trajectory;
